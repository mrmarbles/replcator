 ...
 * @constructor
 */
function Replcator(config) {
  this.options = [];
  this.attributes = [];
  this.prompt = config.prompt || '>> input: ';
  this.config = config;
}
 ...
 * with the Replcator instance.  Once one or more of these
 ...
 * @param callback
 */
Replcator.prototype.on = function(name, callback) {
  this.options.push({
    name: name,
    callback: callback
  });
...
/**
 * Will return a previously registered selectable value (e.g., option)
 * based off of name, if one exists.
 *
 * @param name
 * @return {*}
 */
Replcator.prototype.getOption = function(name) {
...
/**
 * Starts/continues the read-eval-print loop.
 * Avoid invoking this method directly - start()
 * should be used instead.
 *
 * @param option
 */
Replcator.prototype.runner = function(option) {
  var opt, self = this;
  if (opt = this.getOption(option)) {
    this.select(opt);
  } else {
    this.ask(this.getPrompt(), function(input) {
      if (opt = self.getOption(input)) {
        self.select(opt);
      } else {
...
/**
 * Initializes and then starts/continues the read-eval-print loop.
 */
Replcator.prototype.start = function() {
  if (this.config.bootstrap) {
    this.config.bootstrap(this);
  }
  this.setPrompt(this.config.prompt || '>>  input: ');
  if (this.config.start && (opt = this.getOption(this.config.start))) {
    this.select(opt);
 ...
 * @param prompt
 */
Replcator.prototype.setPrompt = function(prompt) {
  this.prompt = prompt;
};

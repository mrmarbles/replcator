...
Replcator.prototype.on = function(matcher, controllerCallback) {
  var self = this;
  this.router.bindPattern(matcher, function(packet, routerCallback) {
...
Replcator.prototype.select = function(option) {
  var self = this;
  self.router.dispatch(option, function(err, packet) {
    if (err && err === 404) {
...
/**
 * Initializes and then starts/continues
...
Replcator.prototype.start = function() {
  ...
  if (start && (this.router.hasMatch(start))) {
    this.select(start);
